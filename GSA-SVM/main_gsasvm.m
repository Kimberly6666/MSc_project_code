...
% Design GSA optim option and visualisation:
gsa_option = struct('Max_iteration',20,'noP',5,'G0',1,...
    'popcmax',10^6,'popcmin',10^(-3),'popgmax',10^1,'popgmin',10^(-1));

[bestAccuracy,cost,gamma,trace] = gsasvm(trainData_hoi,trainLabel_hoi,trainData_oi,trainLabel_oi,testData,testLabel,gsa_option);

figure;
plot(trace,'r-');
xlabel('Generation iteration');
ylabel('Fitness(1-accuracy)');
title('Fitness curve (GSA)')
grid on;
